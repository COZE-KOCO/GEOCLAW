...
${request.articleType !== 'auto' ? `文章类型：${articleTypeNames[request.articleType]}` : '文章类型：根据主题自动判断最合适的类型'}
...
大纲要求：
1. 每个章节标题要包含蒸馏词
2. 结构清晰${request.articleType !== 'auto' ? `，符合${articleTypeNames[request.articleType]}的逻辑` : ''}
3. 每个章节要有明确的信息增量
4. 标题要有吸引力，能引发用户点击
...
export async function generateArticle(
  ...
  // 构建图文笔记专属要求（小红书/抖音）
  const imageTextRequirements: string[] = [];
  if (request.targetPlatforms && request.targetPlatforms.length > 0) {
    const platform = request.targetPlatforms[0];
    
    imageTextRequirements.push(`【图文笔记专属要求】`);
    imageTextRequirements.push(`- 目标平台：${platform === 'xiaohongshu' ? '小红书' : platform === 'douyin' ? '抖音' : platform}`);
    
    // Emoji密度
    const emojiDensityMap = {
      'none': '不使用任何emoji表情',
      'low': '偶尔使用emoji，每段最多1个',
      'medium': '适度使用emoji，每段1-2个，增加亲和力',
      'high': '大量使用emoji，让内容更生动活泼',
    };
    if (request.emojiDensity) {
      imageTextRequirements.push(`- Emoji要求：${emojiDensityMap[request.emojiDensity] || '适度使用'}`);
    }
    
    // 话题标签
    if (request.hashtagCount && request.hashtagCount > 0) {
      imageTextRequirements.push(`- 话题标签：在文末生成${request.hashtagCount}个相关话题标签，格式为 #标签名`);
    }
    
    // 段落风格
    if (request.paragraphStyle === 'short') {
      imageTextRequirements.push(`- 段落风格：短句优先，每句控制在10-20字，便于移动端阅读`);
    }
    
    // 开头钩子
    if (request.enableHook) {
      if (platform === 'xiaohongshu') {
        imageTextRequirements.push(`- 开头钩子：用数字、疑问句或悬念开头，如"3个方法..."、"真的好用吗？"`);
      } else if (platform === 'douyin') {
        imageTextRequirements.push(`- 开头钩子：用冲击性语句或问题开头，吸引前3秒注意力`);
      }
    }
    
    // 结尾引导
    if (request.enableCTA) {
      if (platform === 'xiaohongshu') {
        imageTextRequirements.push(`- 结尾引导：引导互动，如"你觉得呢？"、"评论区告诉我"`);
      } else if (platform === 'douyin') {
        imageTextRequirements.push(`- 结尾引导：引导点赞关注，如"点个赞再走吧"`);
      }
    }
    
    // 平台特定限制
    if (platform === 'xiaohongshu') {
      imageTextRequirements.push(`- 标题限制：不超过20字，要吸睛、数字化或疑问句`);
      imageTextRequirements.push(`- 正文限制：不超过1000字`);
    } else if (platform === 'douyin') {
      imageTextRequirements.push(`- 标题限制：不超过55字`);
      imageTextRequirements.push(`- 正文限制：不超过500字，口语化表达`);
    }
  }
...
${request.articleType !== 'auto' ? `文章类型：${articleTypeNames[request.articleType]}` : '文章类型：根据主题内容自动判断最合适的文章类型'}
...
${imageTextRequirements.length > 0 ? `${imageTextRequirements.join('\n')}\n` : ''}
...
返回JSON格式：
{
  "title": "文章标题（必须包含核心关键词，但要创意独特、吸引眼球，拒绝模板化，25-35字）",
